...
`;function y1(){const{moduleId:e,lessonId:t}=Sp(),n=Ke(),[r,o]=w.useState(null),[i,l]=w.useState(null),[s,a]=w.useState(!0),[c,m]=w.useState("files"),[p,g]=w.useState(null),[x,C]=w.useState(null);return w.useEffect(()=>{(async()=>{try{const f=(await il(()=>import("./ModuleData-CVFXap4d.js"),[])).modules.find(d=>d.id===parseInt(e));if(f){o(f);const d=[...f.lessons].sort((y,E)=>y.order-E.order),h=d.findIndex(y=>y.id===parseInt(t));h!==-1&&(l(d[h]),h>0&&g(d[h-1]),h<d.length-1&&C(d[h+1]))}a(!1)}catch(z){console.error("Erro ao carregar a aula:",z),a(!1)}})()},[e,t]),w.useEffect(()=>{i&&(i.files&&i.files.length>0?m("files"):i.audios&&i.audios.length>0&&m("audios"))},[i]),s?u.jsx(f1,{children:"Carregando aula..."}):!r||!i?u.jsxs("div",{children:[u.jsx(Bc,{variant:"outline",onClick:()=>n(`/modulos/${e}`),children:"← Voltar para o Módulo"}),u.jsx("p",{children:"Aula não encontrada."})]}):u.jsxs("div",{children:[u.jsx(Bc,{variant:"outline",onClick:()=>n(`/modulos/${e}`),children:"← Voltar para o Módulo"}),u.jsxs(a1,{children:[u.jsx(u1,{children:i.title}),u.jsx(c1,{children:u.jsx("iframe",{src:i.videoUrl,title:i.title,allowFullScreen:!0,frameBorder:"0"})}),u.jsxs(d1,{children:[u.jsx(bc,{$active:c==="files",onClick:()=>m("files"),children:"Arquivos"}),u.jsx(bc,{$active:c==="audios",onClick:()=>m("audios"),children:"Áudios"})]}),c==="files"&&u.jsx(s1,{files:i.files}),c==="audios"&&(i.audios&&i.audios.length>0?u.jsx(p1,{children:i.audios.map(k=>u.jsxs(m1,{children:[u.jsx(h1,{children:k.title}),k.description&&u.jsx(g1,{children:k.description}),u.jsxs(v1,{controls:!0,children:[u.jsx("source",{src:k.audioUrl,type:"audio/mp3"}),"Seu navegador não suporta o elemento de áudio."]})]},k.id))}):u.jsx("p",{children:"Nenhum áudio disponível para esta aula."}))]})]})}const x1=v.div`
  max-width: 400px;
  margin: 2rem auto;
  background-color: var(--card-bg);
  border-radius: 12px;
...
`,H1=v.p`
  ...
  border-radius: 4px;
  margin-bottom: 1rem;
`,Uo="/api",ql={username:"admin",password:"admin123"};function Q1(){console.log("AdminModules.jsx atualizado, versão com lessons sanitizado");const e=Ke(),[t,n]=w.useState([]),[r,o]=w.useState(!0),[i,l]=w.useState(!1),[s,a]=w.useState({id:"",title:"",description:"",order:1}),[c,m]=w.useState(""),[p,g]=w.useState(""),x=async()=>{try{o(!0),console.log("Fazendo requisição para /api/modules (AdminModules)");const y=await fetch(`${Uo}/modules`);if(console.log("Status da resposta (AdminModules):",y.status),!y.ok){const j=await y.text();throw console.error("Resposta inválida (AdminModules):",j),new Error("Failed to fetch modules")}const E=await y.json();console.log("Dados recebidos (AdminModules):",JSON.stringify(E)),console.log("Dados são array? (AdminModules)",Array.isArray(E));const S=E.map(j=>({...j,lessons:Array.isArray(j.lessons)?j.lessons:[]}));S.sort((j,L)=>j.order-L.order),n(S)}catch(y){console.error("Erro ao carregar os módulos (AdminModules):",y),m("Ocorreu um erro ao carregar os módulos. Tente novamente.")}finally{o(!1)}};w.useEffect(()=>{x()},[]);const C=()=>{l(!0),a({id:"",title:"",description:"",order:t.length>0?Math.max(...t.map(y=>y.order))+1:1}),m(""),g("")},k=()=>{l(!1),a({id:"",title:"",description:"",order:1}),m("")},z=y=>{const{name:E,value:S}=y.target;a(j=>({...j,[E]:E==="order"?parseInt(S)||1:S}))},f=async y=>{if(y.preventDefault(),m(""),g(""),!s.title){m("Por favor, preencha todos os campos obrigatórios.");return}try{const E={title:s.title,description:s.description,order:s.order,lessons:s.id?void 0:[]};let S;if(s.id?S=await fetch(`${Uo}/modules/${s.id}`,{method:"PUT",headers:{"Content-Type":"application/json",...ql},body:JSON.stringify(E)}):S=await fetch(`${Uo}/modules`,{method:"POST",headers:{"Content-Type":"application/json",...ql},body:JSON.stringify(E)}),!S.ok){const j=await S.text();throw console.error("Erro na resposta (AdminModules):",j),new Error("Failed to save module")}g(s.id?"Módulo atualizado com sucesso!":"Módulo adicionado com sucesso!"),l(!1),setTimeout(()=>{g("")},3e3),await x()}catch(E){console.error("Erro ao salvar o módulo (AdminModules):",E),m("Ocorreu um erro ao salvar o módulo. Tente novamente.")}},d=y=>{a({id:y.id,title:y.title,description:y.description,order:y.order}),l(!0),m(""),g("")},h=async y=>{if(window.confirm("Tem certeza que deseja excluir este módulo? Todas as aulas, arquivos e áudios associados também serão excluídos."))try{const E=await fetch(`${Uo}/modules/${y}`,{method:"DELETE",headers:ql});if(!E.ok){const S=await E.text();throw console.error("Erro na resposta (AdminModules):",S),new Error("Failed to delete module")}g("Módulo excluído com sucesso!"),setTimeout(()=>{g("")},3e3),await x()}catch(E){console.error("Erro ao excluir o módulo (AdminModules):",E),m("Ocorreu um erro ao excluir o módulo. Tente novamente.")}};return r&&t.length===0?u.jsx("p",{children:"Carregando módulos..."}):u.jsxs(L1,{children:[c&&u.jsx(W1,{children:c}),p&&u.jsx(H1,{children:p}),i?u.jsxs(U1,{children:[u.jsx(b1,{children:s.id?"Editar Módulo":"Adicionar Novo Módulo"}),u.jsxs("form",{onSubmit:f,children:[u.jsxs(Kl,{children:[u.jsx(Yl,{htmlFor:"title",children:"Título *"}),u.jsx(Qc,{type:"text",id:"title",name:"title",value:s.title,onChange:z,required:!0})]}),u.jsxs(Kl,{children:[u.jsx(Yl,{htmlFor:"description",children:"Descrição"}),u.jsx(B1,{id:"description",name:"description",value:s.description,onChange:z})]}),u.jsxs(Kl,{children:[u.jsx(Yl,{htmlFor:"order",children:"Ordem"}),u.jsx(Qc,{type:"number",id:"order",name:"order",min:"1",value:s.order,onChange:z})]}),u.jsxs(V1,{children:[u.jsx(Y,{variant:"outline",type:"button",onClick:k,children:"Cancelar"}),u.jsx(Y,{type:"submit",children:s.id?"Atualizar":"Adicionar"})]})]})]}):u.jsxs(u.Fragment,{children:[u.jsxs(I1,{children:[u.jsx("h1",{children:"Gerenciar Módulos"}),u.jsx(Y,{onClick:C,children:"Adicionar Módulo"})]}),u.jsx(N1,{children:t.length>0?t.map(y=>u.jsxs(R1,{children:[u.jsxs(M1,{children:[u.jsx(A1,{children:y.title}),u.jsx(O1,{children:y.description}),u.jsxs(F1,{children:[u.jsxs("span",{children:["Ordem: ",y.order]}),u.jsx("span",{children:" | "}),u.jsxs("span",{children:["Aulas: ",Array.isArray(y.lessons)?y.lessons.length:0]})]})]}),u.jsxs(D1,{children:[u.jsx(Y,{variant:"outline",size:"small",onClick:()=>d(y),children:"Editar"}),u.jsx(Y,{variant:"text",size:"small",onClick:()=>h(y.id),children:"Excluir"}),u.jsx(Y,{variant:"text",size:"small",onClick:()=>e(`/modulos/${y.id}`),children:"Ver"})]})]},y.id)):u.jsx("p",{children:"Nenhum módulo encontrado."})})]})]})}const Ot="/api",vt={username:"admin",password:"admin123"},G1=v.div`
  display: flex;
  flex-direction: column;
  gap: 2rem;
`,K1=v.div`
