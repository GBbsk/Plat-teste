...
`;function y1(){const{moduleId:e,lessonId:t}=Sp(),n=Ke(),[r,o]=w.useState(null),[i,l]=w.useState(null),[s,a]=w.useState(!0),[c,m]=w.useState("files"),[p,g]=w.useState(null),[x,k]=w.useState(null),[C,z]=w.useState({});return w.useEffect(()=>{(async()=>{try{const h=(await il(()=>import("./ModuleData-C5ULGfxf.js"),[])).modules.find(y=>y.id===parseInt(e));if(h){o(h);const y=[...h.lessons].sort((S,E)=>S.order-E.order),j=y.findIndex(S=>S.id===parseInt(t));j!==-1&&(l(y[j]),j>0&&g(y[j-1]),j<y.length-1&&k(y[j+1]))}a(!1)}catch(d){console.error("Erro ao carregar a aula:",d),a(!1)}})()},[e,t]),w.useEffect(()=>{i&&(i.files&&i.files.length>0?m("files"):i.audios&&i.audios.length>0&&m("audios"))},[i]),s?u.jsx(f1,{children:"Carregando aula..."}):!r||!i?u.jsxs("div",{children:[u.jsx(Bc,{variant:"outline",onClick:()=>n(`/modulos/${e}`),children:"← Voltar para o Módulo"}),u.jsx("p",{children:"Aula não encontrada."})]}):u.jsxs("div",{children:[u.jsx(Bc,{variant:"outline",onClick:()=>n(`/modulos/${e}`),children:"← Voltar para o Módulo"}),u.jsxs(a1,{children:[u.jsx(u1,{children:i.title}),u.jsx(c1,{children:u.jsx("iframe",{src:i.videoUrl,title:i.title,allowFullScreen:!0,frameBorder:"0"})}),u.jsxs(d1,{children:[u.jsx(bc,{$active:c==="files",onClick:()=>m("files"),children:"Arquivos"}),u.jsx(bc,{$active:c==="audios",onClick:()=>m("audios"),children:"Áudios"})]}),c==="files"&&u.jsx(s1,{files:i.files}),c==="audios"&&(i.audios&&i.audios.length>0?u.jsx(p1,{children:i.audios.map(f=>u.jsxs(m1,{children:[u.jsx(h1,{children:f.title}),f.description&&u.jsx(g1,{children:f.description}),u.jsxs(v1,{controls:!0,children:[u.jsx("source",{src:f.audioUrl,type:"audio/mp3"}),"Seu navegador não suporta o elemento de áudio."]}),f.transcript&&f.transcript.length>0&&u.jsxs(u.Fragment,{children:[u.jsx(Y,{variant:"outline",size:"small",style:{margin:"0.5rem 0"},onClick:()=>z(d=>({...d,[f.id]:!d[f.id]})),children:C[f.id]?"Ocultar Transcrição":"Mostrar Transcrição"}),C[f.id]&&u.jsxs("div",{className:"audio-transcript",children:[u.jsx("h4",{children:"Transcrição"}),u.jsx("ul",{children:f.transcript.map((d,h)=>u.jsxs("li",{children:[u.jsxs("strong",{children:[d.time,"s:"]})," ",d.text]},h))})]})]})]},f.id))}):u.jsx("p",{children:"Nenhum áudio disponível para esta aula."}))]})]})}const x1=v.div`
  max-width: 400px;
  margin: 2rem auto;
  background-color: var(--card-bg);
  border-radius: 12px;
